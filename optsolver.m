function [x_sol, summary] = optsolver(problem, algorithm, x_initial, kernel, b, i)
    ...
    if strcmp(algorithm, 'douglasrachfordprimal') == 1
        ...
        x_sol = primaldr(b, i.tprimaldr, i.rhoprimaldr, gamma, i.maxiter, x_initial, kernel, norm_prox);

    elseif strcmp(algorithm, 'douglasrachfordprimaldual') == 1
        % need to decompose x_initial to right format for this algorithm
        x_sol = primaldualdr(b, i.tprimaldualdr, i.rhoprimaldualdr, gamma, i.maxiter, x_initial, kernel, norm_prox);

    elseif strcmp(algorithm, 'admm') == 1

    elseif strcmp(algorithm, 'chambollepock') == 1
        % need to decompose x_initial to right format for this algorithm
        x_sol = chambollepock(b, i.tcp, i.scp, gamma, i.maxiter, x_initial, kernel, norm_prox);
